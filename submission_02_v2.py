...
# Reading the dataset
train = pd.read_csv('User/Falakjabin/Desktop/ZS/dataset/train_file.csv')
test = pd.read_csv('User/Falakjabin/Desktop/ZS/dataset/test_file.csv')
sample_submission = pd.read_csv('User/Falakjabin/Desktop/ZS/dataset/sample_submission.csv')
...
model_2.fit(x_train_title_transformed, y_train_title)

y_train_title_pred = model_2.predict(x_train_title_transformed)
mae_title_train = mean_absolute_error(y_train_title, y_train_title_pred)
print("mae_title_train: ", mae_title_train)

y_valid_title_pred = model_2.predict(x_valid_title_transformed)
...
model_2h = RandomForestRegressor(n_estimators=500,
                      max_depth=30,
                      random_state=0, n_jobs = -1)

model_2h.fit(x_train_headline_transformed, y_train_headline)

y_train_headline_pred = model_2h.predict(x_train_headline_transformed)
mae_headline_train = mean_absolute_error(y_train_headline, y_train_headline_pred)
print("mae_headline_train: ", mae_headline_train)

y_valid_headline_pred = model_2h.predict(x_valid_headline_transformed)
...
model_2.fit(x_title_train_transformed, y_title_train)

x_title_test = test['Title']
x_title_test_transformed = text_transformation(x_title_test)
x_title_test = model_2.predict(x_title_test_transformed)
...
model_2h.fit(x_headline_train_transformed, y_headline_train)

x_headline_test = test['Headline']
x_headline_test_transformed = text_transformation(x_headline_test)
x_headline_test = model_2h.predict(x_headline_test_transformed)
